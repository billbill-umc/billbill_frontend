...
interface PostService {
    ...
    @PATCH("/posts/{postId}")
    fun editPost(@Path("postId") postId : Int, @Body editPostRequest: EditPostRequest) : Call<EditPostResponse>

    @DELETE("/posts/{postId}")
    fun deletePost(@Path("postId") postId : Int) : Call<DeletePostResponse>

    @POST("/posts/{postId}/images")
    fun uploadPostImage(@Path("postId") postId : Int) : Call<UploadPostImageResponse>

    @DELETE("/posts/{postId}/images/{imageId}")
    fun deletePostImage(@Path("postId") postId : Int, @Path("imageId") imageId : Int) : Call<DeletePostImageResponse>
...
//        RetrofitClient.instance.getPosts().enqueue(object : Callback<GetPostsResponse> {
